#!/usr/bin/env python3
"""
WF-MCANet D4e — Long training dari awal, 600 epoch
Setara dengan paper MCANet: ~80k iterations
  600ep × 65 batch = 39k iter (batch=32, setara ~78k iter batch=16)
Setup IDENTIK D4b yang sudah terbukti:
  - 537/537 pretrained weights
  - 1-channel BCE + FG-Dice
  - Resize (bukan crop)
  - gate=0.1
HANYA 2 perubahan:
  1. Epochs: 200 → 600
  2. Scheduler: CosineAnnealingLR T_max=600 (1 cycle, no restart)
"""
...
LOG_FILE=BASE_DIR/'train_d4e.log'
...
log('='*65)
log('  WF-MCANet D4e — 600ep, CosLR 1-cycle, identik D4b')
log(f'  Started : {time.strftime("%Y-%m-%d %H:%M:%S")}')
log(f'  GPU     : {torch.cuda.get_device_name(0)}')
log('='*65)

# ============================================================
# Arsitektur IDENTIK D4b (copy paste, tidak ada perubahan)
# ============================================================
...
class MSCAAttention(nn.Module):
    def __init__(self,dim):
        ...
        self.spatial_gating_unit=SpatialGatingUnit(dim); self.proj_2=nn.Conv2d(dim,dim,1)
    def forward(self,x):
...
class MixFFN(nn.Module):
    def __init__(self,d,hd,drop=0.):
        ...
        self.norm=nn.BatchNorm2d(hd); self.fc2=nn.Conv2d(hd,d,1); self.drop=nn.Dropout(drop)
    def forward(self,x):
        x=self.fc1(x); x=self.act(x); x=self.dwconv(x)
        x=self.norm(x); x=self.drop(x); x=self.fc2(x); return self.drop(x)
...
class OPEmbed(nn.Module):
    def __init__(self,ic,oc,stride=2):
        super().__init__()
        self.proj=nn.Conv2d(ic,oc,3,stride=stride,padding=1,bias=True); self.norm=nn.BatchNorm2d(oc)
...
class MSCAN(nn.Module):
    ...
    def _load(self,path):
        ...
        log(f'  MSCAN-T: {len(ok)}/{len(sd)} loaded')
    def forward(self,x):
        outs=[]
        for i in range(4):
...
def total_loss(seg,bnd,t): return 0.5*bce_loss(seg,t)+0.5*dice_loss_fg(seg,t)+edge_loss(bnd,t)
...
def run(name,model,epochs=600,lr=2e-4,wd=1e-4,ev=10):
    tp=sum(p.numel() for p in model.parameters())
    log(f'\n{"="*65}')
    log(f'  {name}  |  Params:{tp/1e6:.3f}M  Epochs:{epochs}')
    log(f'  LR:{lr} → eta_min=1e-6 (1 cosine cycle, no restart)')
    log(f'  Equiv paper iterations: {epochs}×{len(tr_ld)}×{BATCH} = {epochs*len(tr_ld)*BATCH/1000:.0f}k samples')
    log(f'{"="*65}')
    ck=CHECKPOINT_DIR/f'{name}_best.pt'
    opt=AdamW(model.parameters(),lr=lr,weight_decay=wd,betas=(0.9,0.999))
    # 1 cosine cycle across full training — smooth decay, no restart spike
    ...
    for ep in range(1,epochs+1):
        ...
        if do:
            ...
            log(f'Ep {ep:3d}/{epochs} | loss={tl:.4f} | Dice={m["dice"]:.4f} | '
                f'IoU={m["iou"]:.4f} | HD95={hd}px | lr={cur_lr:.1e} | '
                f'gate={model.wfm.gate.item():.3f}{star}')
        else:
    ...
    log(f'  Dice:{fm["dice"]:.4f}  IoU:{fm["iou"]:.4f}  HD95:{fm["hd95"]:.2f}px')
    log(f'{"-"*65}')
    json.dump({'history':dict(hist),'final':fm},open(RESULTS_DIR/f'{name}_log.json','w'),indent=2)
    return fm,model

log('\n>>> D4e: Setup identik D4b, 600 epoch, 1-cycle CosLR <<<')
model=WFMCANet(dc=64,h=8,pretrained=True).to(DEVICE)
results,model=run('D4e_WFMCANet',model,epochs=600,lr=2e-4)
...
rows=[
    ...
    ('D4b (200ep)',         0.9061, 0.8407),
    ('D4e (600ep, ours)',  results['dice'], results['iou']),
]
log(f'{"Method":<28} {"Dice":>7} {"IoU":>7}')
log('-'*45)
for nm,d,i in rows: log(f'{nm:<28} {d:>7.4f} {i:>7.4f}')
log('='*65)
