#!/usr/bin/env python3
"""
WF-MCANet D4c — 4 perbaikan utama dibanding D4b:
  1. Differential LR: backbone=2e-5, decoder=2e-4 (ratio 1:10)
  2. PolynomialLR decay (no restart spikes) + warmup 10 epoch  
  3. Epochs 400 (model butuh lebih banyak iterasi)
  4. Random crop augmentasi (bukan resize saja) seperti paper asli
  Semua fix D4b tetap: 537/537 weights, 1ch sigmoid, fg-only Dice
"""
...
LOG_FILE=BASE_DIR/'train_d4c.log'
...
log('='*65)
log('  WF-MCANet D4c — DiffLR + PolyLR + 400ep + CropAug')
...
# ============================================================
# MSCAN-T (identical to D4b fixed — 537/537 confirmed)
# ============================================================
...
class MixFFN(nn.Module):
    # key: mlp.dwconv (confirmed from D4b log)
...
class StemConv(nn.Module):
    def __init__(self,ic,oc):
        ...
        self.proj=nn.Sequential(
            nn.Conv2d(ic,oc//2,3,stride=2,padding=1,bias=True),
            nn.BatchNorm2d(oc//2), nn.GELU(),
            nn.Conv2d(oc//2,oc,3,stride=2,padding=1,bias=True),
            nn.BatchNorm2d(oc),
        )
...
class MSCAN(nn.Module):
    ...
    def _load(self,path):
        ...
        miss=[k for k in sd if k not in ok]
        self.load_state_dict(ok,strict=False)
        extra=[k for k in msd if k not in sd]
        log(f'  MSCAN-T: {len(ok)}/{len(sd)} loaded | miss={len(miss)} | extra(ok)={len(extra)}')
        if miss: log(f'  WARN miss: {miss[:3]}')
...
class WFMCANet(nn.Module):
    ...
    def get_param_groups(self, lr_backbone, lr_decoder):
        """
        DIFFERENTIAL LR: backbone gets 10x lower LR than decoder.
        Pretrained features are fragile — large updates destroy them.
        Decoder is randomly initialized — needs larger steps.
        """
        backbone_params = list(self.backbone.parameters())
        backbone_ids = set(id(p) for p in backbone_params)
        decoder_params = [p for p in self.parameters() if id(p) not in backbone_ids]
        return [
...
# ============================================================
# Loss (same as D4b — confirmed correct)
# ============================================================
...
# ============================================================
# Dataset — dengan RandomCrop seperti paper asli
# ============================================================
...
tr_t=A.Compose([
    # FIX: RandomResizedCrop agar model lihat lesion di berbagai skala
    # Paper MCANet: random crop 512x512 dari gambar yang di-resize ke 556
    A.SmallestMaxSize(max_size=556),
    A.RandomCrop(IMG_SIZE,IMG_SIZE),
    A.HorizontalFlip(p=0.5),
    A.VerticalFlip(p=0.5),
    A.Rotate(limit=30,p=0.5),
    A.ColorJitter(brightness=0.3,contrast=0.3,saturation=0.15,hue=0.05,p=0.6),
    A.OneOf([
...
def train_ep(model,loader,opt,sc):
    ...
    for imgs,masks in loader:
        ...
        # Gradient clipping — stabilize large batch training
        sc.unscale_(opt)
...
def evaluate(model,loader,tta=True):
    ...
    return {'dice':float(np.mean(dl)),'iou':float(np.mean(il)),
            'hd95':float(np.mean(hl)) if hl else float('nan'),'ms/img':1000*tt/n}

def run(name,model,epochs=400,lr_bb=2e-5,lr_dec=2e-4,wd=1e-4,ev=5):
    tp=sum(p.numel() for p in model.parameters())
    log(f'\n{"="*65}')
    log(f'  {name}')
    log(f'  Params:{tp/1e6:.3f}M  Epochs:{epochs}  Batch:{BATCH}')
    log(f'  LR: backbone={lr_bb} | decoder={lr_dec} (ratio 1:{int(lr_dec/lr_bb)})')
    log(f'  Scheduler: PolynomialLR (no restarts) + 10ep warmup')
    log(f'  Augment: SmallestMaxSize(556)+RandomCrop(512) like paper')
    log(f'{"="*65}')

    ck=CHECKPOINT_DIR/f'{name}_best.pt'
    # DIFFERENTIAL LR: backbone 10x lower than decoder
    param_groups=model.get_param_groups(lr_bb, lr_dec)
    opt=AdamW(param_groups,weight_decay=wd,betas=(0.9,0.999))
    # PolynomialLR: smooth decay, NO restart spikes
    # power=0.9 → aggressive early decay then plateau near end
    sch=PolynomialLR(opt,total_iters=epochs,power=0.9)
    sc=torch.amp.GradScaler('cuda')
    best=0.; hist=defaultdict(list); t0=time.time()

    for ep in range(1,epochs+1):
        # Warmup 10 epochs (longer than D4b's 5)
        if ep<=10:
            warmup_factor=ep/10
            for i,pg in enumerate(opt.param_groups):
                base_lr=lr_bb if i==0 else lr_dec
        ...
        if do:
            ...
            cur_lr_bb=opt.param_groups[0]['lr']; cur_lr_dec=opt.param_groups[1]['lr']
            gate_v=model.wfm.gate.item()
            log(f'Ep {ep:3d}/{epochs} | loss={tl:.4f} | Dice={m["dice"]:.4f} | '
                f'IoU={m["iou"]:.4f} | HD95={hd}px | '
                f'lr_bb={cur_lr_bb:.1e} | gate={gate_v:.3f}{star}')
        else:
    ...
    log(f'{"-"*65}')
    json.dump({'config':name,'history':dict(hist),'final':fm},
...
# Run
# ============================================================
log('\n>>> D4c: DiffLR(1:10) + PolyLR + 400ep + CropAug <<<')
model=WFMCANet(dc=64,h=8,pretrained=True).to(DEVICE)

results,history=run('D4c_WFMCANet',model,epochs=400)
...
log('\n'+'='*65)
rows=[
    ('MCANet-T (paper)',      0.9293, 0.9040, float('nan'), 4.04),
    ('D4b fixed (prev run)', 0.9061, 0.8407, 25.01,        4.50),
    ('D4c (this run)',       results['dice'],results['iou'],results['hd95'],4.50),
]
log(f'{"Method":<30} {"Dice":>7} {"IoU":>7} {"HD95":>7} {"P(M)":>6}')
log('-'*55)
for nm,d,i,h,p in rows:
    hd=f'{h:.2f}' if not math.isnan(h) else '  ---'
    ii=f'{i:.4f}' if not math.isnan(i) else '  ---'
    log(f'{nm:<30} {d:>7.4f} {ii:>7} {hd:>7} {p:>5.2f}M')
log('='*65)
pd.DataFrame([{'method':nm,'dice':d,'iou':i,'hd95':h} for nm,d,i,h,p in rows]
             ).to_csv(RESULTS_DIR/'results_d4c.csv',index=False)
