...
def edge_loss(pred, target, weight=0.7):
    ...
    pl = torch.softmax(pred, 1)[:, 1:2]
    return weight * F.binary_cross_entropy(pl, eg)
...
# 15. Run all 4 ablation experiments

NUM_EPOCHS = 100
results, histories = {}, {}

# Config A — MCANet Baseline
model_A = MCANet(num_classes=2, dec_channels=64, num_heads=8, pretrained=True).to(DEVICE)
results['A'], histories['A'] = run_experiment(
    'A_MCANet_Baseline', model_A, use_edge_loss=False, num_epochs=NUM_EPOCHS)
torch.cuda.empty_cache()

# Config B — MCANet + WFM only
model_B = WFMCANet(num_classes=2, dec_channels=64, num_heads=8, pretrained=True).to(DEVICE)
results['B'], histories['B'] = run_experiment(
    'B_MCANet_WFM_Only', model_B, use_edge_loss=False, num_epochs=NUM_EPOCHS)
torch.cuda.empty_cache()
